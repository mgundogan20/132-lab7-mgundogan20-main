...
public class BookStoreClerk {
	...
	private String[] partitionString(String input) throws InvalidBookException
	{
		...
		if(first == last) { //both -1 for no hyphens, and some n for a single hyphen
			throw new InvalidBookException("Invalid Input to partitionString method: Could not find seperator character '-'", new IllegalArgumentException());
		}
		for(int i=0; i<input.length() ;i++) {
			if(input.charAt(i) == '-' && i!=first && i!=last) {
				//there is a 3rd hyphen, the input is invalid
				throw new InvalidBookException("Invalid Input, too many tokens", new IllegalArgumentException());
			}
		}
	...
	public Book readAndValidateBook() throws InvalidBookException
	{
		...
		if(input.trim().length() <= 0) {
			throw new InvalidBookException("Input BookInfo is empty!", new IllegalArgumentException());
		}
